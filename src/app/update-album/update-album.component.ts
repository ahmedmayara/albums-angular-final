...
export class UpdateAlbumComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.albumService.getAlbumById(this.activatedRoute.snapshot.params['id']).
    subscribe( album =>{ this.currentAlbum = album;
  ...
  updateAlbum() {
    this.currentAlbum.label = this.labels.find(label => label.idLabel == this.updatedLabelId)!;
